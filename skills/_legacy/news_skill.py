...
class NewsSkill:
    ...
    def _fetch_news_data(self, category: str, country: str) -> Dict[str, Any]:
        ...
        params = {"country": country, "apiKey": self._api_key, "pageSize": 10}

        if category:
            params["category"] = category

        response = requests.get(
            f"{self._base_url}/top-headlines", params=params, timeout=10
        )
    ...
    def _search_news_data(self, query: str, max_results: int) -> Dict[str, Any]:
        ...
        response = requests.get(f"{self._base_url}/everything", params=params, timeout=10)
        response.raise_for_status()
        data = response.json()
